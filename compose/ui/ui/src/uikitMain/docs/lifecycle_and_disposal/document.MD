...
time once the object is collected.
...
Undefined behavior in its finest.

## Stayin` alive and dying ASAP while being correct

Since `lifetime` becomes a blurred and smudged term in the mix of two different runtimes, we need to
either:
...
// We will hide [weak self] of CMPViewController in those callbacks on ObjC side.
class CMPViewControllerCallbacks(
    val presentViewController: (UIViewController, Bool) -> Unit
    // and other callbacks, that user needs for the UIViewController
) : NSObject()
...
class CMPViewController: UIViewController {
   ...
   override func loadView() {
       ...
       // ObjC/Swift owns it, while keeping the control block alive, while this lambda wasn't 
       // collected by Kotlin GC
...
from accidently capturing `presentingViewController` in a lambda.
...
### Reusing some determined SDK capabilites to reinvent lifetime
...
We could go the other way and treat attaching and detaching from `window` as event to take decisions
...
There is no way to maintain both invariants:
1. `onDestroy` is called once.
2. `onDestory` is called in determined way. 
