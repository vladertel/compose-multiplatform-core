...
fun Foo() {
    ...
    Button(onClick = {
        callbacks.presentViewControllerFromCmpView(view, true)
    }) {
        Text("Present")
    }
}
...
explore every use case. For example, we don't want a modal presentation, we want to push something onto
navigation stack:
```kotlin
