...
internal actual class ComposeWindow : UIViewController {
    ...
    private val counter = nextCounter()
    ...
    private val density: Density
        get() = Density(composeLayer?.layer?.contentScale ?: 1f, fontScale)

    private var composeLayer: ComposeLayer? = null
    private var skiaLayer: SkiaLayer? = null
    private lateinit var content: @Composable () -> Unit
    private var uiKitTextInputService: UIKitTextInputService? = null
    private var uiKitPlatform: Platform? = null
    ...
    private val keyboardVisibilityListener = object : NSObject() {
        ...
        private fun calcFocusedLiftingY(focusedRect: DpRect, keyboardHeight: Double): Double {
            val hiddenPartOfFocusedElement: Double =
                keyboardHeight - (composeLayer?.layer?.height ?: 0f) + focusedRect.bottom.value
    ...
    override fun loadView() {
        ...
        ).also { uiKitTextInputService = it }

        uiKitPlatform = object : Platform by Platform.Empty {
            ...
            override val textToolbar = object : TextToolbar {
                 ...
                 */
                override fun hide() {
                    skikoUIView?.hideTextMenu()
                }

                override val status: TextToolbarStatus
                    get() = if (skikoUIView?.isTextMenuShown() == true)
    ...
    private fun recreateComposeLayerIfNeeded() {
        if (composeLayer != null) {
            return
        }

        val skiaLayer = createSkiaLayer().also {
            skiaLayer = it
        }
        ...
        val skikoUIView = SkikoUIView(
            ...
            skikoUITextInputTrains = DelegateSkikoUITextInputTraits {
                uiKitTextInputService?.skikoUITextInputTraits
            }
        ).load().also {
            skikoUIView = it
        }
        ...
        val uikitPlatform = uiKitPlatform ?: throw IllegalStateException("uiKitPlatform is null")
        val uiKitTextInputService = uiKitTextInputService ?: throw IllegalStateException("uiKitTextInputService is null")
    ...
    override fun viewWillLayoutSubviews() {
        ...
        composeLayer?.let {
            ...
            it.setSize((width * scale).roundToInt(), (height * scale).roundToInt())
        }
    }
