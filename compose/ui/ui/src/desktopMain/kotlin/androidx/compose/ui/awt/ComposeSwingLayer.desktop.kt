...
internal interface ComposeSwingLayer {
    val component: SkiaSwingLayerComponent
...
internal class ComposeSwingLayerImpl(
    ...
    @OptIn(ExperimentalComposeUiApi::class)
    override var exceptionHandler: WindowExceptionHandler? = null
    ...
    private inner class ComponentImpl :
        ...
        private fun updateSceneSize() {
            this@ComposeSwingLayerImpl.scene.constraints = Constraints(
        ...
        override fun getPreferredSize(): Dimension {
            return if (isPreferredSizeSet) super.getPreferredSize() else Dimension(
                (this@ComposeSwingLayerImpl.scene.contentSize.width / density.density).toInt(),
                (this@ComposeSwingLayerImpl.scene.contentSize.height / density.density).toInt()
            )
        }
        ...
        private fun resetDensity() {
            density = (this as SkiaSwingLayer).density
            if (this@ComposeSwingLayerImpl.scene.density != density) {
                this@ComposeSwingLayerImpl.scene.density = density
                updateSceneSize()
            }
        }
