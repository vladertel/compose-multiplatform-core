 ...
 *
 * Initial size of the dialog is controlled by [DialogState.size].
 * Initial position of the dialog is controlled by [DialogState.position] and [initialAlignment].
 *
 ...
 * If [DialogState.position] is [WindowPosition.isInitial], then after the first show on the screen
 * [DialogState.position] will be set to the absolute values.
 ...
 * @param initialAlignment if not null and If [DialogState.position] is [WindowPosition.isInitial],
 ...
 * it changes implicit [DialogState.isOpen] state
...
//     DialogState.size as it is. If this property is true then we will ignore the size in
//     DialogState and reset it on the attach.
        ...
        Dialog(
            ...
            update = { dialog ->
                updater.update {
                    ...
                    set(state.position) {
                        dialog.setPositionSafely(it, initialAlignment)
                    }
                }
            },
            content = content
 ...
 * Dialog will also be disposed and closed when [DialogState.isOpen] will become `false`.
 *
