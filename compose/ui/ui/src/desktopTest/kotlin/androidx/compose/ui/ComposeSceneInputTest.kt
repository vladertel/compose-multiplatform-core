...
class ComposeSceneInputTest {
    ...
    fun `move to popup`() = ImageComposeScene(100, 100).use { scene ->
        ...
        scene.sendPointerEvent(PointerEventType.Enter, Offset(-10f, -10f))
        background.events.assertReceivedNoEvents()
        cutPopup.events.assertReceivedLast(PointerEventType.Enter, Offset(-10f, -10f) - cutPopup.origin)
        ...
        background.events.assertReceivedNoEvents()
        cutPopup.events.assertReceivedLast(PointerEventType.Exit, Offset(20f, 20f) - cutPopup.origin)
        overlappedPopup.events.assertReceivedLast(PointerEventType.Enter, Offset(20f, 20f) - overlappedPopup.origin)
        independentPopup.events.assertReceivedNoEvents()

        scene.sendPointerEvent(PointerEventType.Move, Offset(80f, 80f))
        background.events.assertReceivedNoEvents()
        cutPopup.events.assertReceivedNoEvents()
        overlappedPopup.events.assertReceivedLast(PointerEventType.Exit, Offset(80f, 80f) - overlappedPopup.origin)
        independentPopup.events.assertReceivedLast(PointerEventType.Enter, Offset(80f, 80f) - independentPopup.origin)
        ...
        cutPopup.events.assertReceivedNoEvents()
        overlappedPopup.events.assertReceivedNoEvents()
        independentPopup.events.assertReceivedLast(PointerEventType.Exit, Offset(90f, 90f) - independentPopup.origin)
    ...
    @Test
    fun `pressed popup should own received moves outside popup`() = ImageComposeScene(100, 100).use { scene ->
        val background = FillBox()
        val cutPopup = PopupState(IntRect(-20, -20, 40, 40))
        val overlappedPopup = PopupState(IntRect(20, 20, 60, 60))
        val independentPopup = PopupState(IntRect(80, 80, 90, 90))
        ...
        cutPopup.events.assertReceivedLast(PointerEventType.Press, Offset(-10f, -10f) - cutPopup.origin)
        overlappedPopup.events.assertReceivedNoEvents()
        independentPopup.events.assertReceivedNoEvents()

        scene.sendPointerEvent(PointerEventType.Enter, Offset(10f, 10f))
        background.events.assertReceivedNoEvents()
        cutPopup.events.assertReceivedLast(PointerEventType.Move, Offset(10f, 10f) - cutPopup.origin)
        overlappedPopup.events.assertReceivedNoEvents()
        independentPopup.events.assertReceivedNoEvents()

        scene.sendPointerEvent(PointerEventType.Move, Offset(20f, 20f))
        background.events.assertReceivedNoEvents()
        cutPopup.events.assertReceivedLast(PointerEventType.Move, Offset(20f, 20f) - cutPopup.origin)
        overlappedPopup.events.assertReceivedNoEvents()
        independentPopup.events.assertReceivedNoEvents()

        scene.sendPointerEvent(PointerEventType.Move, Offset(80f, 80f))
        background.events.assertReceivedNoEvents()
        cutPopup.events.assertReceivedLast(PointerEventType.Move, Offset(80f, 80f) - cutPopup.origin)
        overlappedPopup.events.assertReceivedNoEvents()
        independentPopup.events.assertReceivedNoEvents()

        scene.sendPointerEvent(PointerEventType.Move, Offset(90f, 90f))
        background.events.assertReceivedNoEvents()
        cutPopup.events.assertReceivedLast(PointerEventType.Move, Offset(90f, 90f) - cutPopup.origin)
        overlappedPopup.events.assertReceivedNoEvents()
        independentPopup.events.assertReceivedNoEvents()

        scene.sendPointerEvent(PointerEventType.Move, Offset(100f, 100f))
        background.events.assertReceivedNoEvents()
        cutPopup.events.assertReceivedLast(PointerEventType.Move, Offset(100f, 100f) - cutPopup.origin)
        overlappedPopup.events.assertReceivedNoEvents()
        independentPopup.events.assertReceivedNoEvents()

        scene.sendPointerEvent(PointerEventType.Move, Offset(110f, 110f))
        background.events.assertReceivedNoEvents()
        cutPopup.events.assertReceivedLast(PointerEventType.Move, Offset(110f, 110f) - cutPopup.origin)
        overlappedPopup.events.assertReceivedNoEvents()
        independentPopup.events.assertReceivedNoEvents()
        ...
        background.events.assertReceivedNoEvents()
        cutPopup.events.assertReceivedLast(PointerEventType.Release, Offset(20f, 20f) - cutPopup.origin)
        overlappedPopup.events.assertReceivedNoEvents()
        independentPopup.events.assertReceivedNoEvents()
        ...
        background.events.assertReceivedNoEvents()
        cutPopup.events.assertReceivedNoEvents()
        overlappedPopup.events.assertReceivedLast(PointerEventType.Press, Offset(20f, 20f) - overlappedPopup.origin)
        independentPopup.events.assertReceivedNoEvents()
        ...
        background.events.assertReceivedNoEvents()
        cutPopup.events.assertReceivedLast(PointerEventType.Exit, Offset(-10f, -10f) - cutPopup.origin)
        overlappedPopup.events.assertReceivedLast(PointerEventType.Enter, Offset(-10f, -10f) - overlappedPopup.origin)
        independentPopup.events.assertReceivedNoEvents()

        scene.sendPointerEvent(PointerEventType.Release, Offset(-10f, -10f))
        background.events.assertReceivedNoEvents()
        cutPopup.events.assertReceivedNoEvents()
        overlappedPopup.events.assertReceivedLast(PointerEventType.Release, Offset(-10f, -10f) - overlappedPopup.origin)
        overlappedPopup.events.assertReceivedNoEvents()
        independentPopup.events.assertReceivedNoEvents()
    }
