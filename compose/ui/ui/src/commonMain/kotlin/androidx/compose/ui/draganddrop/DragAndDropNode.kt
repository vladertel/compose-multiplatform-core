...
interface DragAndDropSourceModifierNode : LayoutAwareModifierNode {
    /**
     * Returns a boolean value indicating whether requesting drag and drop transfer is supported.
     *
     * This variable is used to check if the platform supports drag and drop transfer initiated by
     * application, for example via a custom gesture.
     *
 ...
 * sessions.
 *
 * @param onDropTargetValidate A lambda function used to validate the drop target.
 */
@OptIn(InternalComposeUiApi::class)
fun DragAndDropTargetModifierNode(
    onDropTargetValidate: (DragAndDropEvent) -> DragAndDropTarget?
): DragAndDropTargetModifierNode = DragAndDropNode(onDropTargetValidate = onDropTargetValidate)
...
class DragAndDropNode(
    private var onStartTransfer: (DragAndDropSourceScope.(Offset) -> Unit)? = null,
    ...
    override val isRequestDragAndDropTransferSupported: Boolean
        get() = dragAndDropManager.isRequestDragAndDropTransferSupported
