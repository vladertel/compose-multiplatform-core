...
/* internal */ class JsOwners(
    ...
    private fun invalidateIfNeeded() {
        hasPendingDraws = frameClock.hasAwaiters || list.any(JsOwner::needsRender)
        if (hasPendingDraws && !isInvalidationDisabled) {
            invalidate()
        }
    }

    internal val list = LinkedHashSet<JsOwner>()
    private val listCopy = mutableListOf<JsOwner>()
...
*/
    ...
    internal fun register(JsOwner: JsOwner) {
        println("JsOwners.register() LIST: add")
        list.add(JsOwner)
        JsOwner.onNeedsRender = ::invalidateIfNeeded
        JsOwner.onDispatchCommand = ::dispatchCommand
        invalidateIfNeeded()
    }

    internal fun unregister(JsOwner: JsOwner) {
        println("JsOwners.unregister() LIST: remove")
        list.remove(JsOwner)
        JsOwner.onDispatchCommand = null
        JsOwner.onNeedsRender = null
        invalidateIfNeeded()
    }

    fun onFrame(canvas: Canvas, width: Int, height: Int, nanoTime: Long) {
        println("JsOwners.onFrame() ${list}")
        disableInvalidation {
